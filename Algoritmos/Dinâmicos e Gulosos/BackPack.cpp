...
vector<int> BuildBackPack(vector<int> &values, vector<int> &weights, int max_weight, int **V){
    vector<int> S;
    int X = max_weight;
    int j = values.size();
    while(j >= 1 and X > 0){
        if(V[X][j] == values[j-1] + V[X - weights[j-1]][j-1]){
            S.push_back(j);
            X = X - weights[j-1];
        }
        j--;
...
int BackPackTopDownRecursive(int **V, vector<int> &values, vector<int> &weights, int j, int X){
    if(V[X][j] == -1){
        if(weights[j-1] > X) V[X][j] = BackPackTopDownRecursive(V, values, weights, j-1, X);
        else{
            int into = values[j-1] + BackPackTopDownRecursive(V, values, weights, j-1, X-weights[j-1]);
            int not_into = BackPackTopDownRecursive(V, values, weights, j-1, X);
            V[X][j] = max(into, not_into);
        }
    }
    return V[X][j];
}

int BackPackTopDown(vector<int> &values, vector<int> &weights, int max_weight){
    int n = values.size();
    int **V;
    V = new int *[max_weight+1];
    for(int i = 0; i < max_weight+1; i++) V[i] = new int [n+1];
    ...
    return BackPackTopDownRecursive(V, values, weights, n, max_weight);
}
...
int BackPackBottomUp(vector<int> &values, vector<int> &weights, int max_weight){
    ...
    // Descomente esta parte caso queira que seja printada a mochila construída:
    // vector<int> S = BuildBackPack(values, weights, max_weight, V);
    // cout << "Itens na mochila construida Bottom-Up: ";
...
int main(void){
    ...
    cout << "result = " << BackPackTopDown(values, weights, max_weight) << endl;
    cout << "result = " << BackPackBottomUp(values, weights, max_weight) << endl;
    // Descomente esta parte caso queira que seja printada a mochila construída:
    // cout << BackPackBottomUp(values, weights, max_weight) << endl;
}